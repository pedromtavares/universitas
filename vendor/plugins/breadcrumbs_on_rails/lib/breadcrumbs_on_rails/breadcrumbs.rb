  ...
  module Breadcrumbs
    ...
    class SimpleBuilder < Builder
        ...
        @elements.collect do |element|
					element == @elements.first ? render_element(element, :first => true) : render_element(element)
        end.join
      end

      def render_element(element, options = {})
				return "" if compute_path(element) == ""
        content = @context.link_to(compute_name(element), compute_path(element))
				first = if options.delete(:first)
					options.merge!(:class => @options[:first_class])
					@options[:first_class]
				else
					""
				end
				options.merge!(:class => "#{first} #{@options[:active_class]}") if @context.current_page?(compute_path(element))
        if @options[:tag]
          @context.content_tag @options[:tag], options do
						content
					end
        else
          content
